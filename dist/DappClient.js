...
export class DappClient {
     ...
     */
    constructor(walletUrl, options) {
        const { transportMode = TransportMode.POPUP, keymachineUrl = DEFAULT_KEYMACHINE_URL, redirectUrl, sequenceStorage = new WebStorage(), sequenceSessionStorage, randomPrivateKeyFn, redirectUrlActionHandler, canUseIndexedDb = true, } = options || {};
        this.transport = new DappTransport(walletUrl, transportMode, undefined, sequenceSessionStorage, redirectUrlActionHandler);
     ...
     * });
     */
    async connect(chainId, implicitSessionRedirectUrl, permissions, options = {}) {
     ...
     * }
     */
    async addExplicitSession(chainId, permissions) {
     ...
     * }
     */
    async modifyExplicitSession(chainId, sessionAddress, permissions) {
        if (!this.isInitialized || !this.walletAddress)
    ...
    async disconnect() {
        ...
        this.transport = new DappTransport(this.walletUrl, transportMode, undefined, this.sequenceSessionStorage, this.redirectUrlActionHandler);
    ...
    getChainSessionManager(chainId) {
        ...
        if (!chainSessionManager) {
            chainSessionManager = new ChainSessionManager(chainId, this.keymachineUrl, this.transport, this.sequenceStorage, this.redirectUrl, this.randomPrivateKeyFn, this.canUseIndexedDb);
