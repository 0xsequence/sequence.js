...
export function decodePrecondition(p) {
    ...
    try {
        ...
        switch (p.type) {
            case 'native-balance':
                precondition = new NativeBalancePrecondition(Address.from(data.address), data.min ? BigInt(data.min) : undefined, data.max ? BigInt(data.max) : undefined);
                break;
            case 'erc20-balance':
                precondition = new Erc20BalancePrecondition(Address.from(data.address), Address.from(data.token), data.min ? BigInt(data.min) : undefined, data.max ? BigInt(data.max) : undefined);
                break;
            case 'erc20-approval':
                precondition = new Erc20ApprovalPrecondition(Address.from(data.address), Address.from(data.token), Address.from(data.operator), BigInt(data.min));
                break;
            case 'erc721-ownership':
                precondition = new Erc721OwnershipPrecondition(Address.from(data.address), Address.from(data.token), BigInt(data.tokenId), data.owned);
                break;
            case 'erc721-approval':
                precondition = new Erc721ApprovalPrecondition(Address.from(data.address), Address.from(data.token), BigInt(data.tokenId), Address.from(data.operator));
                break;
            case 'erc1155-balance':
                precondition = new Erc1155BalancePrecondition(Address.from(data.address), Address.from(data.token), BigInt(data.tokenId), data.min ? BigInt(data.min) : undefined, data.max ? BigInt(data.max) : undefined);
                break;
            case 'erc1155-approval':
                precondition = new Erc1155ApprovalPrecondition(Address.from(data.address), Address.from(data.token), BigInt(data.tokenId), Address.from(data.operator), BigInt(data.min));
                break;
            default:
                return undefined;
        }
