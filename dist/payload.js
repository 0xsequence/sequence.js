...
import { EXECUTE_USER_OP } from './constants.js';
...
export function isCalls4337_07(payload) {
    return payload.type === 'call_4337_07';
}
export function isParented(payload) {
...
export function encode4337Nonce(key, seq) {
    if (key > 6277101735386680763835789423207666416102355444464034512895n)
        throw new RangeError('key exceeds 192 bits');
    if (seq > 18446744073709551615n)
        throw new RangeError('seq exceeds 64 bits');
    return (key << 64n) | seq;
}
...
export function to4337UserOperation(payload, wallet, signature) {
    const callsPayload = {
        type: 'call',
        space: 0n,
        nonce: 0n,
...
export function to4337Message(payload, wallet, chainId) {
    const operation = to4337UserOperation(payload, wallet);
    const accountGasLimits = Hex.concat(Hex.padLeft(Hex.fromNumber(operation.verificationGasLimit), 16), Hex.padLeft(Hex.fromNumber(operation.callGasLimit), 16));
    const gasFees = Hex.concat(Hex.padLeft(Hex.fromNumber(operation.maxPriorityFeePerGas), 16), Hex.padLeft(Hex.fromNumber(operation.maxFeePerGas), 16));
    const initCode_hashed = Hash.keccak256(operation.factory && operation.factoryData ? Hex.concat(operation.factory, operation.factoryData) : '0x');
    const paymasterAndData_hashed = Hash.keccak256(operation.paymaster
        ? Hex.concat(operation.paymaster, Hex.padLeft(Hex.fromNumber(operation.paymasterVerificationGasLimit || 0), 16), Hex.padLeft(Hex.fromNumber(operation.paymasterPostOpGasLimit || 0), 16), operation.paymasterData || '0x')
        : '0x');
    ...
    return AbiParameters.encode([{ type: 'bytes32' }, { type: 'address' }, { type: 'uint256' }], [Hash.keccak256(packedUserOp), payload.entrypoint, BigInt(chainId)]);
}
