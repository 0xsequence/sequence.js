...
export async function recover(signature, wallet, chainId, payload, options) {
    if (signature.suffix?.length) {
        ...
        for (const subsignature of signature.suffix) {
            const recovered = await recover(subsignature, wallet, subsignature.noChainId ? 0 : chainId, fromConfigUpdate(Bytes.toHex(hashConfiguration(configuration))), options);
