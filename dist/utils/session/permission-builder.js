...
function parseSignature(sig) {
    ...
    const types = parts.map((p) => {
        const t = p.split(/\s+/)[0];
        if (!t)
            throw new Error(`Invalid parameter in signature: "${p}"`);
        return t;
    });
...
export class PermissionBuilder {
    ...
    addRule(param, expectedType, mask, operation, rawValue, cumulative = false) {
        const offset = this.findOffset(param, expectedType);
        // turn bigint → padded 32-byte, or Bytes → padded‐left 32-byte
        const value = typeof rawValue === 'bigint' ? Bytes.fromNumber(rawValue, { size: 32 }) : Bytes.padLeft(Bytes.from(rawValue), 32);
        this.rules.push({ cumulative, operation, value, offset, mask });
        return this;
    }
    withUintNParam(param, value, bits = 256, operation = Permission.ParameterOperation.EQUAL, cumulative = false) {
        const typeName = `uint${bits}`;
        const mask = Permission.MASK[`UINT${bits}`];
        return this.addRule(param, typeName, mask, operation, value, cumulative);
    }
    withIntNParam(param, value, bits = 256, operation = Permission.ParameterOperation.EQUAL, cumulative = false) {
        const typeName = `int${bits}`;
        const mask = Permission.MASK[`INT${bits}`];
        return this.addRule(param, typeName, mask, operation, value, cumulative);
    }
    withBytesNParam(param, value, size = 32, operation = Permission.ParameterOperation.EQUAL, cumulative = false) {
        const typeName = `bytes${size}`;
        const mask = Permission.MASK[`BYTES${size}`];
        return this.addRule(param, typeName, mask, operation, value, cumulative);
    }
    withAddressParam(param, value, operation = Permission.ParameterOperation.EQUAL, cumulative = false) {
        return this.addRule(param, 'address', Permission.MASK.ADDRESS, operation, Bytes.padLeft(Bytes.fromHex(value), 32), cumulative);
    }
    withBoolParam(param, value, operation = Permission.ParameterOperation.EQUAL, cumulative = false) {
    ...
    withDynamicAtOffset(pointerOffset, value) {
...
export class ERC20PermissionBuilder {
    static buildTransfer(target, limit) {
    ...
    static buildApprove(target, spender, limit) {
        return PermissionBuilder.for(target)
...
export class ERC1155PermissionBuilder {
    static buildTransfer(target, tokenId, limit) {
        return PermissionBuilder.for(target)
    ...
    static buildApproveAll(target, operator) {
        return PermissionBuilder.for(target)
