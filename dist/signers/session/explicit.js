import { Permission, SessionSignature, Constants } from '@0xsequence/wallet-primitives';
...
export class Explicit {
    ...
    async findSupportedPermission(wallet, _chainId, call, sessionManagerAddress, provider) {
        if (call.value !== 0n) {
            // Validate the value
            if (!provider) {
    ...
    async signCall(wallet, chainId, call, nonce, sessionManagerAddress, provider) {
        let permissionIndex;
        if (Hex.size(call.data) > 4 &&
            Hex.isEqual(Hex.slice(call.data, 0, 4), AbiFunction.getSelector(Constants.INCREMENT_USAGE_LIMIT))) {
    ...
    async prepareIncrements(wallet, chainId, calls, sessionManagerAddress, provider) {
        ...
        return Promise.all(increments.map(async ({ usageHash, increment }) => {
            if (increment === 0n)
                return null;
