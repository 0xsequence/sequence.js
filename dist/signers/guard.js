import { Bytes, TypedData, Signature, Hash } from 'ox';
...
export class Guard {
    ...
    async signEnvelope(envelope, token) {
        ...
        const signature = await this.guard.signPayload(envelope.wallet, envelope.chainId, payloadType, digest, message, previousSignatures, token ? { id: token.id, token: token.code, resetAuth: token.resetAuth } : undefined);
        return {
...
function toGuardPayload(wallet, chainId, payload) {
...
function toGuardSignature(signature) {
