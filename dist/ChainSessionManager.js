...
import { AbiFunction, Address, Provider, RpcTransport, Secp256k1 } from 'ox';
import { Envelope, Signers, State, Wallet, Constants, Extensions, SessionConfig, } from './index.js';
import { AddExplicitSessionError, FeeOptionError, InitializationError, ModifyExplicitSessionError, SessionConfigError, TransactionError, WalletRedirectError, } from './utils/errors.js';
...
export class ChainSessionManager {
     ...
     */
    constructor(chainId, transport, projectAccessKey, keyMachineUrl, nodesUrl, relayerUrl, sequenceStorage, redirectUrl, guard, randomPrivateKeyFn, canUseIndexedDb = true) {
        ...
        this.relayer = new RpcRelayer.RpcRelayer(getRelayerUrl(chainId, relayerUrl), this.chainId, getRpcUrl(chainId, nodesUrl, projectAccessKey), undefined, projectAccessKey);
     ...
     */
    initializeWithWallet(walletAddress) {
     ...
     */
    async _loadSessionFromStorage(walletAddress) {
        this.initializeWithWallet(walletAddress);
        const implicitSession = await this.sequenceStorage.getImplicitSession();
        if (implicitSession) {
            await this._initializeImplicitSessionInternal(implicitSession.pk, walletAddress, implicitSession.attestation, implicitSession.identitySignature, false, implicitSession.loginMethod, implicitSession.userEmail, implicitSession.guard);
        }
        const allExplicitSessions = await this.sequenceStorage.getExplicitSessions();
        const walletExplicitSessions = allExplicitSessions.filter((s) => Address.isEqual(Address.from(s.walletAddress), walletAddress) && s.chainId === this.chainId);
        for (const sessionData of walletExplicitSessions) {
            await this._initializeExplicitSessionInternal(sessionData.pk, sessionData.loginMethod, sessionData.userEmail, sessionData.guard, true);
        }
    }
    /**
     ...
     */
    async createNewSession(origin, sessionConfig, options = {}) {
        if (this.isInitialized) {
        ...
        const newSignerAddress = shouldCreateSession && newPk ? Address.fromPublicKey(Secp256k1.getPublicKey({ privateKey: newPk })) : null;
        const completeSession = shouldCreateSession && newSignerAddress
            ? {
        ...
        try {
            if (!this.transport)
                throw new InitializationError('Transport failed to initialize.');
            const payload = {
                origin,
            ...
            const connectResponse = await this.transport.sendRequest(RequestActionType.CREATE_NEW_SESSION, this.redirectUrl, payload, { path: '/request/connect' });
            ...
            if (shouldCreateSession) {
                ...
                if (attestation && signature && newPk) {
                    await this._initializeImplicitSessionInternal(newPk, receivedAddress, attestation, signature, true, loginMethod, userEmail, guard);
                }
    ...
    async addExplicitSession(explicitSessionConfig) {
        if (!this.walletAddress) {
            throw new InitializationError('Cannot add an explicit session without a wallet address. Initialize the manager with a wallet address first.');
        }
        ...
        try {
            ...
            const response = await this.transport.sendRequest(RequestActionType.ADD_EXPLICIT_SESSION, this.redirectUrl, payload, { path: '/request/connect' });
            ...
            await this._initializeExplicitSessionInternal(newPk, response.loginMethod, response.userEmail, response.guard, true);
            await this.sequenceStorage.saveExplicitSession({
                pk: newPk,
     ...
     */
    async modifyExplicitSession(modifiedExplicitSession) {
        if (!this.walletAddress) {
            throw new InitializationError('Cannot modify an explicit session without a wallet address. Initialize the manager with a wallet address first.');
        }
        try {
            if (!this.transport)
                throw new InitializationError('Transport failed to initialize.');
            ...
            const existingExplicitSession = this.explicitSessions.find((s) => Address.isEqual(s.sessionAddress, modifiedExplicitSession.sessionAddress));
            ...
            const response = await this.transport.sendRequest(RequestActionType.MODIFY_EXPLICIT_SESSION, this.redirectUrl, payload, { path: '/request/modify' });
            if (!Address.isEqual(Address.from(response.walletAddress), this.walletAddress) &&
     ...
     */
    async _handleRedirectConnectionResponse(response) {
        try {
            ...
            if (response.action === RequestActionType.CREATE_NEW_SESSION) {
                ...
                if (implicitSessionRequested) {
                    ...
                    await this._initializeImplicitSessionInternal(tempPk, receivedAddress, attestation, signature, true, loginMethod, userEmail, guard);
                }
                ...
                await this._initializeExplicitSessionInternal(explicitSessionPk, this.loginMethod ?? undefined, this.userEmail ?? undefined, this.guard ?? undefined, true);
                await this.sequenceStorage.saveExplicitSession({
     ...
     */
    async _initializeImplicitSessionInternal(pk, address, attestation, identitySignature, saveSession = false, loginMethod, userEmail, guard) {
        if (!this.sessionManager)
            throw new InitializationError('Manager not instantiated for implicit session.');
        try {
            const implicitSigner = new Signers.Session.Implicit(pk, attestation, identitySignature, this.sessionManager.address);
     ...
     */
    async _initializeExplicitSessionInternal(pk, loginMethod, userEmail, guard, allowRetries = false) {
    ...
    async _refreshExplicitSession(expiredSignerAddress) {
        ...
        if (!explicitSigner)
            throw new ModifyExplicitSessionError('Explicit session not found.');
        // Update the deadline
     ...
     */
    async hasPermission(transactions) {
        if (!this.wallet || !this.sessionManager || !this.provider || !this.isInitialized) {
            return false;
        }
        try {
     ...
     */
    async buildSignAndSendTransactions(transactions, feeOption) {
        if (!this.wallet || !this.sessionManager || !this.provider || !this.isInitialized)
            throw new InitializationError('Session is not initialized.');
        try {
            const calls = transactions.map((tx) => ({
            ...
            if (feeOption) {
                if (feeOption.token.contractAddress === Constants.ZeroAddress) {
                    ...
                    callsToSend.unshift({
                        ...
                        data: AbiFunction.encodeData(forwardValue, [feeOption.to, BigInt(feeOption.value)]),
    ...
    response) {
        ...
        if ('payload' in response && response.payload) {
            if (response.action === RequestActionType.CREATE_NEW_SESSION ||
                ...
                const modifyResponse = response.payload;
     ...
     */
    async _buildAndSignCalls(calls) {
        ...
        try {
            ...
            if (preparedIncrement) {
                if (Address.isEqual(this.sessionManager.address, Extensions.Dev1.sessions) ||
            ...
            const parentedEnvelope = {
                ...envelope.payload,
                parentWallets: [this.wallet.address],
            };
            const imageHash = await this.sessionManager.imageHash;
            if (imageHash === undefined)
                throw new SessionConfigError('Session manager image hash is undefined');
            const signature = await this.sessionManager.signSapient(this.wallet.address, this.chainId, parentedEnvelope, imageHash);
            ...
            if (!Envelope.reachedThreshold(signedEnvelope) && this.guard?.moduleAddresses.has(signature.address)) {
                const guard = new Signers.Guard(new Guard.Sequence.Guard(this.guard.url, this.guard.moduleAddresses.get(signature.address)));
     ...
     */
    async _waitForTransactionReceipt(opHash, chainId) {
        try {
            while (true) {
                const currentStatus = await this.relayer.status(opHash, chainId);
                if (currentStatus.status === 'confirmed' || currentStatus.status === 'failed')
                    return currentStatus;
                await new Promise((resolve) => setTimeout(resolve, 1500));
            }
        }
        catch (err) {
            throw new TransactionError(`Transaction failed waiting for receipt: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    /**
     ...
     */
    async _resetStateAndClearCredentials() {
        this._resetState();
