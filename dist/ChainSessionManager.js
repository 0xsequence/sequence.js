...
import { Envelope, Relayer, Signers, State, Wallet, Constants, Extensions, SessionConfig, } from './index.js';
...
export class ChainSessionManager {
    ...
    constructor(chainId, transport, projectAccessKey, keyMachineUrl, nodesUrl, relayerUrl, sequenceStorage, redirectUrl, guard, randomPrivateKeyFn, canUseIndexedDb = true) {
        ...
        this.relayer = new Relayer.Standard.Rpc.RpcRelayer(getRelayerUrl(chainId, relayerUrl), this.chainId, getRpcUrl(chainId, nodesUrl, projectAccessKey));
    ...
    async _handleRedirectConnectionResponse(response) {
        ...
        try {
            ...
            if (response.action === RequestActionType.CREATE_NEW_SESSION) {
                ...
                await this._initializeExplicitSessionInternal(tempPk, this.loginMethod ?? undefined, this.userEmail ?? undefined, this.guard ?? undefined, true);
                await this.sequenceStorage.saveExplicitSession({
                    pk: tempPk,
                    walletAddress: receivedAddress,
                    chainId: this.chainId,
