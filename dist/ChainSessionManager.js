...
export class ChainSessionManager {
     ...
     */
    async createNewSession(implicitSessionRedirectUrl, permissions, options = {}) {
        if (this.isInitialized) {
        ...
        try {
            ...
            const connectResponse = await this.transport.sendRequest(RequestActionType.ADD_IMPLICIT_SESSION, payload, { path: '/request/connect', redirectUrl: this.redirectUrl });
    ...
    async addExplicitSession(permissions) {
        ...
        try {
            ...
            const response = await this.transport.sendRequest(RequestActionType.ADD_EXPLICIT_SESSION, payload, { path: '/request/connect', redirectUrl: this.redirectUrl });
    ...
    async modifyExplicitSession(sessionAddress, newPermissions) {
        ...
        try {
            ...
            const response = await this.transport.sendRequest(RequestActionType.MODIFY_EXPLICIT_SESSION, payload, { path: '/request/modify', redirectUrl: this.redirectUrl });
            if (!Address.isEqual(Address.from(response.walletAddress), this.walletAddress) &&
    ...
    async _requestSignature(action, payload) {
        ...
        try {
            if (this.transport.mode === TransportMode.REDIRECT) {
                ...
                const response = await this.transport.sendRequest(action, payload, {
                    path: '/request/sign',
                    redirectUrl: this.redirectUrl,
                });
...
//# sourceMappingURL=ChainSessionManager.js.map