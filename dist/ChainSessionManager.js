...
export class ChainSessionManager {
     ...
     */
    constructor(chainId, keyMachineUrl, transport, sequenceStorage, redirectUrl, randomPrivateKeyFn, canUseIndexedDb = true) {
     ...
     */
    async _loadSessionFromStorage(implicitSession) {
        ...
        if (implicitSession.chainId === this.chainId) {
            await this._initializeImplicitSessionInternal(implicitSession.pk, walletAddr, implicitSession.attestation, implicitSession.identitySignature, false, implicitSession.loginMethod, implicitSession.userEmail);
        }
        const allExplicitSessions = await this.sequenceStorage.getExplicitSessions();
        const walletExplicitSessions = allExplicitSessions.filter((s) => Address.isEqual(Address.from(s.walletAddress), walletAddr) && s.chainId === this.chainId);
    ...
    async createNewSession(origin, permissions, options = {}) {
        ...
        try {
            ...
            const connectResponse = await this.transport.sendRequest(RequestActionType.ADD_IMPLICIT_SESSION, this.redirectUrl, payload, { path: '/request/connect' });
            ...
            await this._initializeImplicitSessionInternal(newPk, receivedAddress, attestation, signature, true, loginMethod, email);
            if (permissions) {
    ...
    async _handleRedirectConnectionResponse(response) {
        ...
        try {
            ...
            if (response.action === RequestActionType.ADD_IMPLICIT_SESSION) {
                ...
                if (!attestation || !signature)
                    throw new WalletRedirectError('Attestation or signature missing.');
                const savedRequest = await this.sequenceStorage.peekPendingRequest();
                const savedPayload = savedRequest?.payload;
                await this._resetStateAndClearCredentials();
                this.initializeWithWallet(receivedAddress);
                await this._initializeImplicitSessionInternal(tempPk, receivedAddress, attestation, signature, true, loginMethod, email);
     ...
     */
    async _initializeExplicitSessionInternal(pk, allowRetries = false) {
    ...
    async _buildAndSignCalls(calls) {
        ...
        try {
            const preparedIncrement = await this.sessionManager.prepareIncrement(this.wallet.address, BigInt(this.chainId), calls);
            ...
            const signature = await this.sessionManager.signSapient(this.wallet.address, BigInt(this.chainId), parentedEnvelope, imageHash);
