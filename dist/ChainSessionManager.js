...
import { AddExplicitSessionError, FeeOptionError, InitializationError, ModifyExplicitSessionError, SessionConfigError, SigningError, TransactionError, WalletRedirectError, } from './utils/errors.js';
...
export class ChainSessionManager {
     ...
     */
    constructor(chainId, transport, projectAccessKey, keyMachineUrl, nodesUrl, relayerUrl, sequenceStorage, redirectUrl, randomPrivateKeyFn, canUseIndexedDb = true) {
    ...
    async _loadSessionFromStorage(walletAddress) {
        ...
        for (const sessionData of walletExplicitSessions) {
            await this._initializeExplicitSessionInternal(sessionData.pk, sessionData.loginMethod, sessionData.userEmail, true);
        }
    }
    /**
    ...
    async createNewSession(origin, permissions, options = {}) {
        ...
        try {
            ...
            if (attestation && signature) {
                ...
                await this._initializeImplicitSessionInternal(newPk, receivedAddress, attestation, signature, true, loginMethod, userEmail);
            }
            if (permissions) {
                this.initializeWithWallet(receivedAddress);
    ...
    async _handleRedirectConnectionResponse(response) {
        ...
        try {
            ...
            if (response.action === RequestActionType.CREATE_NEW_SESSION) {
                ...
                this.initializeWithWallet(receivedAddress);
                if (attestation && signature) {
                    await this._initializeImplicitSessionInternal(tempPk, receivedAddress, attestation, signature, true, loginMethod, userEmail);
                }
                ...
                await this._initializeExplicitSessionInternal(tempPk, this.loginMethod ?? undefined, this.userEmail ?? undefined, true);
                await this.sequenceStorage.saveExplicitSession({
                    pk: tempPk,
                    walletAddress: receivedAddress,
                    chainId: this.chainId,
     ...
     */
    async _initializeImplicitSessionInternal(pk, address, attestation, identitySignature, saveSession = false, loginMethod, userEmail) {
     ...
     */
    async _initializeExplicitSessionInternal(pk, loginMethod, userEmail, allowRetries = false) {
    ...
    response) {
        ...
        if ('payload' in response && response.payload) {
            ...
            else if (response.action === RequestActionType.SIGN_MESSAGE ||
     ...
     * @throws {SigningError} If the signature request fails.
     */
    async _requestSignature(action, payload) {
        ...
        try {
            if (this.transport.mode === TransportMode.REDIRECT) {
                ...
                const response = await this.transport.sendRequest(action, this.redirectUrl, payload, {
                    path: '/request/sign',
                });
