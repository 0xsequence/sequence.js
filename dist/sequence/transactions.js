...
export class Transactions {
    ...
    async define(transactionId, changes) {
        ...
        // Get relayer options
        const allRelayerOptions = await Promise.all(this.shared.sequence.relayers
            // Filter relayers based on the chainId of the transaction
            .filter((relayer) => relayer instanceof Relayer.Rpc.RpcRelayer ? BigInt(relayer.chainId) === tx.envelope.chainId : true)
            .map(async (relayer) => {
