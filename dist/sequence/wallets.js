            ...
            if (loginEmail && commitment.target) {
                ...
                if (walletEntry) {
                    const updatedWalletEntry = {
                        ...walletEntry,
                        loginEmail,
                        loginType: ('login-' + commitment.kind),
            ...
            if (existingWallets.length > 0) {
                ...
                if (result) {
                    ...
                    if (existingWalletEntry) {
                        ...
                        console.warn('[Wallets/signUp] Wallet selected via UI handler not found in manager DB, or result format unexpected. Selected:', selectedWalletAddress);
                    }
    ...
    async logout(wallet, options) {
        ...
        const nextDevicesTopology = buildCappedTree([
            ...Config.getSigners(devicesTopology)
                .signers.filter((x) => x !== '0x0000000000000000000000000000000000000000' && x.toLowerCase() !== device.address.toLowerCase())
                .map((x) => ({ address: x })),
