...
export class Signatures {
    ...
    onSignatureRequestStatus(status, requestId, callback) {
        let disposed = false;
        const unsubscribe = this.onSignatureRequestUpdate(requestId, (request) => {
            if (disposed)
                return;
    ...
    async request(envelope, action, options = {}) {
        ...
        if (Payload.isConfigUpdate(envelope.payload)) {
            const pendingRequests = await this.shared.databases.signatures.list();
            const pendingConfigUpdatesToClear = pendingRequests.filter((sig) => Address.isEqual(sig.wallet, envelope.wallet) && Payload.isConfigUpdate(sig.envelope.payload));
            ...
            const cancellationResults = await Promise.allSettled(pendingConfigUpdatesToClear.map((sig) => this.shared.modules.signatures.cancel(sig.id)));
            cancellationResults.forEach((result, index) => {
                if (result.status === 'rejected') {
                    const failedSigId = pendingConfigUpdatesToClear[index]?.id;
                    console.error(`Failed to cancel conflicting signature request ${failedSigId || 'unknown ID'} during logout preparation:`, result.reason);
                }
            });
        }
        const id = uuidv7();
