...
export const getEOAWallet = (privateKey: string, providerRpcUrl?: string): EOAWallet => {
  // defaults
  if (!providerRpcUrl) {
  ...
  return (new EOAWallet(privateKey)).connect(provider)
}

export const sendETH = (eoaWallet: EOAWallet, toAddress: string, amount: string): Promise<ethers.providers.TransactionResponse> => {
  // amount is specified without decimals, therefore, "1" will be sent as 1 * 10^18

  const value = ethers.utils.parseEther(amount)
  console.log('weeeeeeeee', value)
