...
import { ethers, AbstractSigner, JsonRpcProvider, hexlify, randomBytes, getBytes } from 'ethers'
...
describe('Signature tools', function () {
  ...
  describe('Change signature configuration', () => {
    ;[
      ...
      },
      {
        ...
        getNewConfig: (config: WalletConfig) => {
          return { threshold: config.threshold, signers: [config.signers[0]] }
        }
      },
      {
      ...
      it(`Should mutate signature after ${c.name}`, async () => {
  ...
  describe('Should generate stub signatures', async () => {
    ...
    it('Should generate stub signature with 4 signers with lower weight', async () => {
      ...
      expect(stub.signers.filter(s => isDecodedAddress(s)).map((s: any) => s.address)).to.deep.equal([
        signer3.address,
        signer5.address
      ])
