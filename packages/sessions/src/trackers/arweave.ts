...
export class ArweaveReader implements ConfigTracker, migrator.PresignedMigrationTracker {
  ...
  constructor(
    ...
    eip5719Provider?: ethers.providers.Provider
  ) {
    if (eip5719Provider) {
  ...
  async loadPresignedConfiguration(args: {
    ...
    const wallet = ethers.utils.getAddress(args.wallet)
    ...
    const fromCheckpoint = ethers.BigNumber.from(fromConfig.checkpoint)

    const items = Object.entries(
      await findItems({ Type: 'config update', Wallet: wallet }, { namespace: this.namespace })
    ).flatMap(([id, tags]) => {
      try {
        const { Signer: signer, Subdigest: subdigest, Digest: digest, 'To-Config': toImageHash } = tags

        let toCheckpoint: ethers.BigNumber
        try {
          toCheckpoint = ethers.BigNumber.from(tags['To-Checkpoint'])
        } catch {
          throw new Error(`to checkpoint is not a number: ${tags['To-Checkpoint']}`)
        }
        if (toCheckpoint.lte(fromCheckpoint)) {
          return []
        }
        ...
        if (!ethers.utils.isHexString(subdigest, 32)) {
          throw new Error(`subdigest is not a hash: ${subdigest}`)
        }

        if (!ethers.utils.isHexString(digest, 32)) {
          throw new Error(`digest is not a hash: ${digest}`)
        }

        let chainId: ethers.BigNumber
        try {
          chainId = ethers.BigNumber.from(tags['Chain-ID'])
        } catch {
          throw new Error(`chain id is not a number: ${tags['Chain-ID']}`)
        }

        if (!ethers.utils.isHexString(toImageHash, 32)) {
          throw new Error(`to config is not a hash: ${toImageHash}`)
        }

        return [{ id, signer, subdigest, digest, chainId, toImageHash, toCheckpoint }]
      } catch (error) {
    ...
    if (args.longestPath) {
      candidates.sort(({ toCheckpoint: a }, { toCheckpoint: b }) => a.sub(b).toNumber())
    } else {
      candidates.sort(({ toCheckpoint: a }, { toCheckpoint: b }) => b.sub(a).toNumber())
    }
    ...
    for (let currentConfig = fromConfig; candidates.length; ) {
      ...
      for (const candidate of candidates) {
        ...
        if (weight.gte(currentConfig.threshold)) {
      ...
      if (!nextCandidate) {
        console.warn(
          `unreachable configs with checkpoint > ${ethers.BigNumber.from(currentConfig.checkpoint).toString()} from config ${currentImageHash}`
        )
        break
      }
      ...
      try {
          ...
          (
            await Promise.all(
              nextCandidateSigners.map(async signer => {
                const { id, subdigest } = nextCandidateItems.get(signer)!
                try {
                  let signature = await (await fetchItem(id)).text()
                  if (this.eip5719) {
                    try {
                      signature = ethers.utils.hexlify(await this.eip5719.runByEIP5719(signer, subdigest, signature))
                    } catch (error) {
        ...
        if (weight.lt(currentConfig.threshold)) {
          throw new Error(
            `insufficient signing power ${weight.toString()} < ${ethers.BigNumber.from(currentConfig.threshold).toString()}`
          )
        }
        updates.push({ wallet, signature, nextImageHash })

        currentConfig = nextConfig
        candidates = candidates.filter(({ toCheckpoint }) => toCheckpoint.gt(currentConfig.checkpoint))
      } catch (error) {
        console.warn(
  ...
  async configOfImageHash(args: { imageHash: string; noCache?: boolean }): Promise<commons.config.Config | undefined> {
    ...
    const config = (async (imageHash: string): Promise<commons.config.Config | undefined> => {
      const items = Object.entries(await findItems({ Type: 'config', Config: imageHash }, { namespace: this.namespace })).flatMap(
        ([id, tags]) => {
          try {
      ...
      switch (items.length) {
        ...
        case 1:
          break
        default:
          console.warn(`multiple configs ${imageHash} at ${items.map(({ id }) => id).join(', ')}, using first`)
          break
      }
  ...
  async imageHashOfCounterfactualWallet(args: {
    ...
    const items = Object.entries(await findItems({ Type: 'wallet', Wallet: wallet }, { namespace: this.namespace })).flatMap(
      ([id, tags]) => {
        try {
  ...
  async walletsOfSigner(args: {
    signer: string
    noCache?: boolean
  }): Promise<Array<{ wallet: string; proof: { digest: string; chainId: ethers.BigNumber; signature: string } }>> {
    const signer = ethers.utils.getAddress(args.signer)

    const proofs: Map<string, { digest: string; chainId: ethers.BigNumber; signature: Promise<string> }> = new Map()

    for (const [id, tags] of Object.entries(
      await findItems({ Type: ['signature', 'config update'], Signer: signer, Witness: 'true' }, { namespace: this.namespace })
    )) {
      ...
      try {
        ...
        proofs.set(wallet, {
          ...
          signature: fetchItem(id).then(response => response.text())
        })
      } catch (error) {
  ...
  async getMigration(
    ...
    const items = Object.entries(
      await findItems(
        {
          Type: 'migration',
          Migration: wallet,
          'Chain-ID': ethers.BigNumber.from(chainId).toString(),
      ...
      try {
        ...
        console.warn(
          `chain ${ethers.BigNumber.from(chainId).toString()} migration ${id} for v${fromVersion} wallet ${wallet} from config ${fromImageHash} invalid:`,
          error
        )
        return []
      }
    ...
    const mainModule = new ethers.utils.Interface(MAIN_MODULE_ABI)
...
async function findItems(
  filter: { [name: string]: string | string[] },
  options?: { namespace?: string; pageSize?: number; maxResults?: number }
): Promise<{ [id: string]: { [tag: string]: string } }> {
  const namespace = options?.namespace
  const pageSize = options?.pageSize ?? 100
  const maxResults = options?.maxResults

  const tags = Object.entries(filter).map(
    ([name, values]) =>
  ...
  for (let hasNextPage = true; hasNextPage && (maxResults === undefined || edges.length < maxResults); ) {
    const query = `
      query {
        transactions(sort: HEIGHT_DESC, ${edges.length ? `first: ${pageSize}, after: "${edges[edges.length - 1].cursor}"` : `first: ${pageSize}`}, tags: [${tags.join(', ')}]) {
          pageInfo {
            hasNextPage
          }
          edges {
    ...
    while (true) {
      ...
      console.warn(`rate limited by arweave.net, trying again in ${RATE_LIMIT_RETRY_DELAY / 1000} seconds at ${new Date(Date.now() + RATE_LIMIT_RETRY_DELAY).toLocaleTimeString()}`)
