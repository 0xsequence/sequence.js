 ...
 */
export function getIdentitySigner(topology: SessionsTopology): Address | null {
  if (isIdentitySignerLeaf(topology)) {
    // Got it
 ...
 */
export function getImplicitBlacklist(topology: SessionsTopology): Address[] | null {
...
export function getSessionPermissions(topology: SessionsTopology, address: Address): SessionPermissions | null {
...
export function getExplicitSigners(topology: SessionsTopology): Address[] {
  return getExplicitSignersFromBranch(topology, [])
}

function getExplicitSignersFromBranch(topology: SessionsTopology, current: Address[]): Address[] {
  if (isSessionPermissions(topology)) {
    return [...current, topology.signer]
  }
  if (isSessionsBranch(topology)) {
    const result: Address[] = [...current]
    for (const child of topology) {
...
export function decodeLeafFromBytes(bytes: Bytes.Bytes): SessionLeaf {
  ...
  if (flag === SESSIONS_FLAG_BLACKLIST) {
    ...
    for (let i = 1; i < bytes.length; i += 20) {
      blacklist.push(normalize(Bytes.toHex(bytes.slice(i, i + 20))))
    }
    return { type: 'implicit-blacklist', blacklist }
  }
  if (flag === SESSIONS_FLAG_IDENTITY_SIGNER) {
    return { type: 'identity-signer', identitySigner: normalize(Bytes.toHex(bytes.slice(1, 21))) }
  }
...
function sessionsTopologyFromParsed(parsed: any): SessionsTopology {
  ...
  if (typeof parsed === 'object' && parsed !== null && 'identitySigner' in parsed) {
    const identitySigner = normalize(parsed.identitySigner)
    return { type: 'identity-signer', identitySigner }
  }

  // Parse blacklist
  if (typeof parsed === 'object' && parsed !== null && 'blacklist' in parsed) {
    const blacklist = parsed.blacklist.map(normalize)
    return { type: 'implicit-blacklist', blacklist }
  }
 ...
 */
export function removeExplicitSession(topology: SessionsTopology, signerAddress: Address): SessionsTopology | null {
  if (isSessionPermissions(topology)) {
 ...
 */
export function addToImplicitBlacklist(topology: SessionsTopology, address: Address): SessionsTopology {
  const blacklistNode = getImplicitBlacklistLeaf(topology)
  if (!blacklistNode) {
 ...
 */
export function removeFromImplicitBlacklist(topology: SessionsTopology, address: Address): SessionsTopology {
  const blacklistNode = getImplicitBlacklistLeaf(topology)
  if (!blacklistNode) {
    throw new Error('No blacklist found')
  }
 ...
 */
export function emptySessionsTopology(identitySigner: Address): SessionsTopology {
  return [
    {
      type: 'implicit-blacklist',
      blacklist: [],
