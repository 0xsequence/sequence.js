...
export function decode(bytes: Bytes.Bytes): Attestation {
  const approvedSigner = normalize(Bytes.toHex(bytes.slice(0, 20)))
...
export function fromParsed(parsed: any): Attestation {
  return {
    approvedSigner: normalize(parsed.approvedSigner),
...
export function generateImplicitRequestMagic(attestation: Attestation, wallet: Address): Bytes.Bytes {
