...
export async function doHash(wallet: Address.Address, chainId: bigint, payload: string): Promise<string> {
  const decoded = AbiParameters.decode(
    [{ type: 'tuple', name: 'payload', components: DecodedAbi }],
    payload as Hex.Hex,
  )[0] as unknown as Payload.SolidityDecoded
  ...
  builder: (yargs) => {
    return yargs
      ...
      .command(
        ...
        async (argv) => {
          const payload = await fromPosOrStdin(argv, 'payload')
          const result = await doConvertToPacked(payload, argv.wallet ? Address.normalize(argv.wallet) : undefined)
          console.log(result)
        },
      )
      .command(
      ...
      .command(
        ...
        async (argv) => {
          const payload = await fromPosOrStdin(argv, 'payload')
          const result = await doHash(Address.normalize(argv.wallet), BigInt(argv.chainId), payload)
          console.log(result)
        },
      )
