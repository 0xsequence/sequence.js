...
export async function doEncodeSignature(options) {
    ...
    const decodedSignature = {
        ...
        clientDataJSON: typeof options.clientDataJson === 'string' ? options.clientDataJson : JSON.stringify(options.clientDataJson),
...
export async function doDecodeSignature(encodedSignatureHex) {
    const encodedBytes = Bytes.fromHex(encodedSignatureHex);
    ...
    const jsonFriendlyDecoded = {
        ...
        publicKey: {
            ...decoded.publicKey,
            metadata: typeof decoded.publicKey.metadata === 'string'
...
const passkeysCommand = {
    command: 'passkeys',
    describe: 'Passkeys extension utilities',
    builder: (yargs) => {
        return yargs
            .command('encode-signature', 'Encode a passkey signature', (yargs) => {
            return yargs
            ...
            .command('decode-signature [encoded-signature]', 'Decode an encoded passkey signature', (yargs) => {
            ...
            .command('root', 'Compute the root hash of a passkey public key tree', (yargs) => {
            return yargs
                .option('x', { type: 'string', description: 'Public key X coordinate (hex)', demandOption: true })
                .option('y', { type: 'string', description: 'Public key Y coordinate (hex)', demandOption: true })
                .option('require-user-verification', {
            ...
            .command('validate-signature', 'Validate a passkey signature', (yargs) => {
            return yargs
