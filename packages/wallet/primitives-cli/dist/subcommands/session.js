...
export async function doEncodeSessionCallSignatures(sessionTopologyInput, callSignaturesInput, explicitSigners = [], implicitSigners = []) {
    const sessionTopology = SessionConfig.sessionsTopologyFromJson(sessionTopologyInput);
    const callSignatures = callSignaturesInput.map((s) => SessionSignature.sessionCallSignatureFromJson(s));
    const encoded = SessionSignature.encodeSessionCallSignatures(callSignatures, sessionTopology, explicitSigners, implicitSigners);
    return Hex.from(encoded);
}
...
const sessionCommand = {
    ...
    builder: (yargs) => {
        return yargs
            .command('empty [identity-signer]', 'Create an empty session topology with the given identity signer', (yargs) => {
            ...
            .command('encode-topology [session-topology]', 'Encode a session topology', (yargs) => {
            ...
            .command('encode-calls [session-topology] [call-signatures] [explicit-signers] [implicit-signers]', 'Encode call signatures for sessions', (yargs) => {
            return yargs
            ...
            console.log(await doEncodeSessionCallSignatures(args.sessionTopology, args.callSignatures, args.explicitSigners, args.implicitSigners));
        })
            .command('image-hash [session-topology]', 'Hash a session topology', (yargs) => {
            return yargs.positional('session-topology', {
                type: 'string',
                description: 'The session topology',
                demandOption: true,
