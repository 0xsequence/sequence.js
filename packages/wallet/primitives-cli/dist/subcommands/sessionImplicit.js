...
export async function doAddBlacklistAddress(blacklistAddress, sessionTopologyInput) {
...
export async function doRemoveBlacklistAddress(blacklistAddress, sessionTopologyInput) {
    const sessionTopology = SessionConfig.sessionsTopologyFromJson(sessionTopologyInput);
    const updated = SessionConfig.removeFromImplicitBlacklist(sessionTopology, blacklistAddress);
    return SessionConfig.sessionsTopologyToJson(updated);
}
const sessionImplicitCommand = {
    command: 'implicit',
    describe: 'Implicit session utilities',
    builder: (yargs) => {
        return yargs
            .command('blacklist-add [blacklist-address] [session-topology]', 'Add an address to the implicit session blacklist', (yargs) => {
            return yargs
            ...
            .command('blacklist-remove [blacklist-address] [session-topology]', 'Remove an address from the implicit session blacklist', (yargs) => {
            return yargs
                .positional('blacklist-address', {
                type: 'string',
                description: 'Blacklist address',
...
//# sourceMappingURL=sessionImplicit.js.map