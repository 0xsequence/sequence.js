import { SessionConfig, Config } from '@0xsequence/wallet-primitives';
...
async function generateSessionsTopology(depth, options) {
    ...
    if (isLeaf || depth <= 1) {
        ...
        const permissions = await Promise.all(Array.from({ length: permissionsCount }, () => generateRandomPermission(options)));
        return {
...
async function generateRandomPermission(options) {
...
async function generateRandomRule(options) {
    return {
...
export async function doRandomConfig(maxDepth, options) {
    const config = {
...
export async function doRandomSessionTopology(maxDepth, options) {
...
const command = {
    ...
    builder: (yargs) => yargs
        .command('random-config', 'Generate a random configuration', (yargs) => {
        return yargs
            .option('max-depth', {
            type: 'number',
        ...
        const options = {
            ...
            checkpointerMode: argv.checkpointer,
        };
        const result = await doRandomConfig(argv.maxDepth, options);
        console.log(result);
    })
        .command('random-session-topology', 'Generate a random session topology', (yargs) => {
        return yargs
            .option('max-depth', {
            type: 'number',
        ...
        const result = await doRandomSessionTopology(argv.maxDepth, options);
        console.log(result);
    })
