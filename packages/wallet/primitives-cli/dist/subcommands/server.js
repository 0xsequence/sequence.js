...
const rpcMethods = {
    ...
    async session_encodeCallSignatures(params) {
        ...
        const result = await session.doEncodeSessionCallSignatures(JSON.stringify(sessionTopology), callSignatures.map(JSON.stringify), explicitSigners, implicitSigners);
        return result;
    },
    ...
    async signature_encode(params) {
        ...
        const result = await signatureUtils.doEncode(JSON.stringify(input), signatures.split(' '), !chainId, checkpointerData);
        return result;
    },
...
async function handleSingleRequest(rpcRequest, debug, silent) {
    const { id, jsonrpc, method, params } = rpcRequest;
    if (!silent)
        console.log(`[${new Date().toISOString()}] Processing request: method=${method} id=${id}`);
    if (debug && !silent) {
    ...
    if (jsonrpc !== '2.0') {
        ...
        if (!silent)
            console.log(`[${new Date().toISOString()}] Error response:`, debug ? JSON.stringify(error, null, 2) : error.error.message);
        return error;
    }
    ...
    try {
        ...
        if (!silent)
            console.log(`[${new Date().toISOString()}] Success response for method=${method} id=${id}`);
        if (debug && !silent) {
...
async function handleHttpRequest(req, res, debug, silent) {
    if (!silent)
        console.log(`[${new Date().toISOString()}] ${req.method} ${req.url} from ${req.socket.remoteAddress}`);
    // Only handle POST /rpc
    if (req.method !== 'POST' || req.url !== '/rpc') {
        if (!silent)
            console.log(`[${new Date().toISOString()}] 404 Not Found`);
        res.statusCode = 404;
        res.end('Not Found');
        return;
    }
    ...
    try {
        ...
        if (!silent)
            console.log(`[${new Date().toISOString()}] JSON parse error:`, error);
        res.statusCode = 400;
    ...
    if (Array.isArray(rpcRequests)) {
        if (!silent)
            console.log(`[${new Date().toISOString()}] Processing batch request with ${rpcRequests.length} items`);
...
async function startServer(host, port, debug, silent) {
