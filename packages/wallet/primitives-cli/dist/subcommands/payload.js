...
export async function doConvertToAbi(_payload) {
    // Not implemented yet, but following the pattern
    throw new Error('Not implemented');
}
export async function doConvertToPacked(payload, wallet) {
    const decodedPayload = Payload.fromAbiFormat(AbiParameters.decode([{ type: 'tuple', name: 'payload', components: DecodedAbi }], payload)[0]);
    if (Payload.isCalls(decodedPayload)) {
        const packed = Payload.encode(decodedPayload, wallet ? wallet : undefined);
        return Hex.from(packed);
    }
    throw new Error('Not implemented');
}
export async function doConvertToJson(payload) {
    const decoded = AbiParameters.decode([{ type: 'tuple', name: 'payload', components: DecodedAbi }], payload)[0];
    const json = JSON.stringify(decoded);
    return json;
}
export async function doHash(wallet, chainId, payload) {
    const decoded = AbiParameters.decode([{ type: 'tuple', name: 'payload', components: DecodedAbi }], payload)[0];
...
const payloadCommand = {
    ...
    builder: (yargs) => {
        return yargs
            ...
            .command('to-packed [payload] [wallet]', 'Convert payload to packed format', (yargs) => {
            return yargs
                .positional('payload', {
                type: 'string',
                description: 'Input payload to convert',
            ...
            .command('to-json [payload]', 'Convert payload to JSON format', (yargs) => {
            return yargs.positional('payload', {
                type: 'string',
                description: 'Input payload to convert',
            });
            ...
            .command('hash [payload]', 'Hash the payload', (yargs) => {
            return yargs
                .option('wallet', {
                type: 'string',
                description: 'Wallet of the wallet to hash the payload',
