...
async function parseLeaves(leavesInput) {
...
export async function doHashFromLeaves(leavesInput) {
...
export async function doEncode(leavesInput) {
    const leaves = await parseLeaves(leavesInput);
    const topology = Extensions.Recovery.fromRecoveryLeaves(leaves);
    const encoded = Extensions.Recovery.encodeTopology(topology);
    return Bytes.toHex(encoded);
}
export async function doTrim(leavesInput, signer) {
    const leaves = await parseLeaves(leavesInput);
    const topology = Extensions.Recovery.fromRecoveryLeaves(leaves);
...
export async function doHashEncoded(encodedStr) {
...
const recoveryCommand = {
    ...
    builder: (yargs) => {
        return yargs
            .command('hash-from-leaves [leaves...]', 'Compute the hash of a recovery topology from leaves', (yargs) => {
            return yargs
                .positional('leaves', {
                type: 'string',
                array: true,
            ...
            .command('encode [leaves...]', 'Encode recovery leaves into topology bytes', (yargs) => {
            ...
            .command('trim [leaves...]', 'Trim the topology to a specific signer and encode', (yargs) => {
            return yargs
                .positional('leaves', {
                type: 'string',
                array: true,
            ...
            .command('hash-encoded [encoded]', 'Compute the hash of an encoded recovery topology', (yargs) => {
