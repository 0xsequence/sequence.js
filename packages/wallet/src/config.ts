...
//
// subDigest = packMessageData(wallet.address, chainId, keccak256(message))
export const recoverConfig = async (
  ...
  const digest = getBytes(keccak256(subDigest))
...
export const recoverConfigFromDigest = async (
  ...
  const decoded =
    (<DecodedSignature>signature).threshold !== undefined ? <DecodedSignature>signature : decodeSignature(signature as string)

  const signers = await Promise.all(
    decoded.signers.map(async s => {
      if (isDecodedEOASigner(s)) {
        ...
        if (walletSignersValidation) {
          if (!(await isValidSignature(s.address, getBytes(digest), hexlify(s.signature), provider, context, chainId)))
