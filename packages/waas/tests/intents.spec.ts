...
describe('Payloads', () => {
  it('Should sign a payload', async () => {
    ...
    expect(signedIntent.signatures[0].signature).to.equal('0x14682ca0eb116109cdf1d0bad6a84e29787787b4a1779d2b43c28d8705ade929267474e8a7725d5e7540ded2010897d3ecaad32b27c75fbfb4f63ff1cf1a948a1c')
  })
  ...
  it('Should sign a message payload', async () => {
    ...
    expect(signedIntent.signatures.length).to.equal(1)
    expect(signedIntent.signatures[0].sessionId).to.equal(await session.sessionId())
    expect(signedIntent.signatures[0].signature).to.equal('0x768b25315317e551ed7b540e73fdf69d8816dcc763a50c648cf2966849f089a2495103f06c876c502bfb33cb348c4b77ffe39bbd6483b932b806a5817374f9ea1c')
  })
  ...
  it('Should sign transaction payload', async () => {
    ...
    expect(signedIntent.signatures.length).to.equal(1)
    expect(signedIntent.signatures[0].sessionId).to.equal(await session.sessionId())
    expect(signedIntent.signatures[0].signature).to.equal('0x98dd84b3d4fe077b2f55e2839609b226d8119b9b0ee10756122615a5d68746bf60596069a305a7533123f212b576d16f3f14ad06faed9fc005c32a28bf8bafb21b')
  })
})
