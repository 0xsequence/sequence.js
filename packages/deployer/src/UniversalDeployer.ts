...
export class UniversalDeployer {
  ...
  deployUniversalDeployer = async (txParams?: TransactionRequest) => {
  ...
  deployUniversalDeployer2 = async (txParams?: TransactionRequest) => {
  ...
  addressOf = async <T extends ContractFactory>(
    ...
    const codeHash = keccak256(solidityPacked(['bytes'], [deployData]))

    const salt = solidityPacked(['uint256'], [contractInstance])
