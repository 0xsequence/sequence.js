  ...
  while (arr.length > 0) {
    ...
    switch (type) {
      case SignaturePartType.Signature:
        {
          const weight = arr[0]
          const signature = ethers.toBeHex(arr.slice(1, SignaturePartTypeLength + 1))
      ...
      case SignaturePartType.Node:
        {
          const nodeHash = ethers.toBeHex(arr.slice(0, 32))
      ...
      case SignaturePartType.Subdigest:
        {
          const subdigest = ethers.toBeHex(arr.slice(0, 32))
      ...
      default:
        throw new Error(`Unknown signature part type: ${type}: ${ethers.toBeHex(arr)}`)
    }
  }
