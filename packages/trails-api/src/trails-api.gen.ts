...
export const WebrpcSchemaVersion = 'v1-25.11.17+25340a94'

// Schema hash generated from your RIDL schema
export const WebrpcSchemaHash = '6d939222fe9ec82e442ef3a769f80523fe7603bc'
...
export interface TrailsClient {
   ...
   */
  getPopularTokens(req: GetPopularTokensRequest, headers?: object, signal?: AbortSignal): Promise<GetPopularTokensResponse>
   ...
   */
  searchTokens(req: SearchTokensRequest, headers?: object, signal?: AbortSignal): Promise<SearchTokensResponse>
...
export interface GetPopularTokensRequest {
  routeProvider?: string
  limit?: number
  chainIds?: Array<number>
}

export interface GetPopularTokensResponse {
  tokens: Array<TokenInfo>
}
...
export class Trails implements TrailsClient {
  ...
  queryKey = {
    ...
    getPopularTokens: (req: GetPopularTokensRequest) => ['Trails', 'getPopularTokens', req] as const,
    searchTokens: (req: SearchTokensRequest) => ['Trails', 'searchTokens', req] as const,
  ...
  getPopularTokens = (
    req: GetPopularTokensRequest,
    headers?: object,
    signal?: AbortSignal
  ): Promise<GetPopularTokensResponse> => {
    return this.fetch(this.url('GetPopularTokens'), createHttpRequest(JsonEncode(req), headers, signal)).then(
      res => {
        return buildResponse(res).then(_data => {
          return JsonDecode<GetPopularTokensResponse>(_data, 'GetPopularTokensResponse')
        })
      },
      error => {
        throw WebrpcRequestFailedError.new({ cause: `fetch(): ${error instanceof Error ? error.message : String(error)}` })
  ...
  searchTokens = (req: SearchTokensRequest, headers?: object, signal?: AbortSignal): Promise<SearchTokensResponse> => {
    return this.fetch(this.url('SearchTokens'), createHttpRequest(JsonEncode(req), headers, signal)).then(
      res => {
        return buildResponse(res).then(_data => {
          return JsonDecode<SearchTokensResponse>(_data, 'SearchTokensResponse')
        })
      },
      error => {
        throw WebrpcRequestFailedError.new({ cause: `fetch(): ${error instanceof Error ? error.message : String(error)}` })
...
export const WebrpcHeaderValue = 'webrpc@v0.31.2;gen-typescript@v0.23.1;trails-api@v1-25.11.17+25340a94'
